...
namespace AlexMedia.Utils
{
    ...
    ///
    ///Custom: supports IDictionaryAdapters and direct access to dict
    /// 
    ...
    /// </summary>
    public class CustomDictionarySource : Source
    {
        /// <inheritdoc />>
        public override bool TryEvaluateSelector(ISelectorInfo selectorInfo)
        {
            ...
            if (current is IDictionaryAdapter iDictionaryAdapter)
            {
                current = iDictionaryAdapter.This.Dictionary;
            }
            ...
            {
                selectorInfo.Result = rawDict[selector];
                return true;
            }
        ...
        protected internal readonly IDictionary<Type, (PropertyInfo, PropertyInfo)?> RoDictionaryTypeCache =
            SmartSettings.IsThreadSafeMode
                ? (IDictionary<Type, (PropertyInfo, PropertyInfo)?>)new ConcurrentDictionary<Type, (PropertyInfo, PropertyInfo)?>()
                : (IDictionary<Type, (PropertyInfo, PropertyInfo)?>)new Dictionary<Type, (PropertyInfo, PropertyInfo)?>();
        ...
        private bool TryGetDictionaryProperties(Type type,
            out (PropertyInfo KeyProperty, PropertyInfo ItemProperty)? propertyTuple)
        {
            ...
            System.Diagnostics.Debug.Assert(
                propertyTuple.Value.KeyProperty != null && propertyTuple.Value.ItemProperty != null,
                "Key and Item properties must not be null");
